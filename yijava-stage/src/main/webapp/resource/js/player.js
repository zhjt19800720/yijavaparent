...
function(b, f) {
	...
	b.jPlayer.event = {};
	b.each("ready flashreset resize repeat click error warning loadstart progress suspend abort emptied stalled play pause loadedmetadata loadeddata waiting playing canplay canplaythrough seeking seeked timeupdate ended ratechange durationchange volumechange".split(" "),
	function() {
	...
	var n = function(a) {
		...
		function(b, c) {
			if (a.target.nodeName.toUpperCase() === c.toUpperCase()) return d = !0,
			!1
	...
	b.jPlayer.prototype = {
		...
		options: {
			...
			cssSelectorAncestor: "#cnc_container",
			cssSelector: {
				videoPlay: ".cnc-video-play",
				play: ".cnc-play",
				pause: ".cnc-pause",
				stop: ".cnc-stop",
				seekBar: ".cnc-seek-bar",
				playBar: ".cnc-play-bar",
				mute: ".cnc-mute",
				unmute: ".cnc-unmute",
				volumeBar: ".cnc-volume-bar",
				volumeBarValue: ".cnc-volume-bar-value",
				volumeMax: ".cnc-volume-max",
				currentTime: ".cnc-current-time",
				duration: ".cnc-duration",
				fullScreen: ".cnc-full-screen",
				restoreScreen: ".cnc-restore-screen",
				repeat: ".cnc-repeat",
				repeatOff: ".cnc-repeat-off",
				gui: ".cnc-gui",
				noSolution: ".cnc-no-solution"
			},
			smoothPlayBar: !1,
			fullScreen: !1,
			fullWindow: !1,
		...
		optionsVideo: {
			size: {
				width: "480px",
				height: "270px",
				cssClass: "cnc-video-270p"
			},
			sizeFull: {
				width: "100%",
				height: "100%",
				cssClass: "cnc-video-full"
			}
		},
		instances: {},
		status: {
		...
		_init: function() {
			...
			this.internal.instance = "cnc_" + this.count;
		...
		_seeking: function() {
			this.css.jq.seekBar.length && this.css.jq.seekBar.addClass("cnc-seeking-bg")
		},
		_seeked: function() {
			this.css.jq.seekBar.length && this.css.jq.seekBar.removeClass("cnc-seeking-bg")
		},
		_resetGate: function() {
		...
		_cssSelector: function(a, c) {
			var d = this;
			"string" === typeof c ? b.jPlayer.prototype.options.cssSelector[a] ? (this.css.jq[a] && this.css.jq[a].length && this.css.jq[a].unbind(".jPlayer"), this.options.cssSelector[a] = c, this.css.cs[a] = this.options.cssSelectorAncestor + " " + c, this.css.jq[a] = c ? b(this.css.cs[a]) : [], this.css.jq[a].length && this.css.jq[a].bind("click.jPlayer",
			function(c) {
				c.preventDefault();
				d[a](c);
				b(this).blur()
		...
		_setOption: function(a, c) {
			...
			case "timeFormat":
				...
				break;
			case "keyEnabled":
				this.options[a] = c; ! c && this === b.jPlayer.focus && (b.jPlayer.focus = null);
				break;
			case "keyBindings":
