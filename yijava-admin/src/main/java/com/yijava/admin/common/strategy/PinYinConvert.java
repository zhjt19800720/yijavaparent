 ...
 *
 */
public abstract class PinYinConvert {
	
	/**
	 * 获取汉字转换拼音首字母后的字符串
	 * 
	 * @param strTextPass 字符串
	 * @param onlyChineseCharacters 是否仅汉字为首字母其他如:英文，数值等一律不转化
	 * 
	 * @return String
	 */
	public static String getAcronym(String strTextPass,boolean onlyChineseCharacters) {
		List<String> letters = getLetter(strTextPass,onlyChineseCharacters);
		StringBuffer buffer = new StringBuffer();
		if(onlyChineseCharacters) {
			for (Iterator<String> it = letters.iterator(); it.hasNext();) {
				buffer.append(it.next());
			}
		} else {
			for (Iterator<String> it = letters.iterator(); it.hasNext();) {
				buffer.append(it.next().charAt(0));
			}
		}
	 ...
	 * @return {@link Vector}
	 */
	public static String getFull(String strTextPass) {
		List<String> letters = getLetter(strTextPass,false);
		StringBuffer buffer = new StringBuffer();
		for (Iterator<String> it = letters.iterator(); it.hasNext();) {
			buffer.append(it.next());
		}
	 ...
	 * @return {@link Vector}
	 */
	public static List<String> getLetter(String strTextPass,boolean acronym) {
		if (StringUtils.isEmpty(strTextPass)) {
		...
		for (int i = 0; i < strText.length; i++) {
			...
			strValue = "";
			while (t < 0x80 && t >= 0) {
				if (Character.isLetter((char) t) || Character.isDigit((char) t)) {
					strValue = (strValue + (char) strText[i]).toUpperCase();
				} else {
			...
			letter.add(acronym ? strRes.charAt(0) + "" : strRes);
			i++;
		}
		return letter;
	}
