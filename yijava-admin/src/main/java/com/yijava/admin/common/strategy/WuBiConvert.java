 ...
 *
 */
public abstract class WuBiConvert {
	
	/**
	 * 五笔文字排版
	 */
	private static final String[] textComposition = new String[] {
	 ...
	 * @return String
	 */
	public static String getAcronym(String strTextPass) {
		if (StringUtils.isEmpty(strTextPass)) {
			return "";
		}
		strTextPass = strTextPass.toUpperCase();
		StringBuffer result = new StringBuffer();
		// 用char循环取得每一个String的 字符
		for (int i = 0; i < strTextPass.length(); i++) {
			char temp = strTextPass.charAt(i);
			...
			if (asciicode >= 1 && asciicode <= 254) {
				...
				for (int j = 0; j < textComposition.length; j++) {
					char[] dbTmp = textComposition[j].toCharArray();
					// 循环遍历每个英文字母的字库
					for (int k = 1; k < dbTmp.length; k++) {
						// 找到
						if ((int) temp == (int) dbTmp[k]) {
							// 如果在WB最后一行，表明是中文标点符号，直接显示，否则显示对应的五笔首字母(该行的第1个字符)
							if (j == textComposition.length - 1) {
