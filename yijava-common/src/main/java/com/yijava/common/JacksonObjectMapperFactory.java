...
public class JacksonObjectMapperFactory implements FactoryBean<ObjectMapper> {
	 ...
	 private Logger logger = LoggerFactory.getLogger(this.getClass());
	    ...
	    /**
	     * 使用国际标准化组织的<a href="http://zh.wikipedia.org/wiki/ISO_8601" >国际标准ISO</a> 8601是日期和时间的表示方法，全称为
	    ...
	    private List<JsonSerializer<?>> jsonSerializers = new ArrayList<JsonSerializer<?>>(5);
	    
//	    private JsonSerializer<StoreId> jsonStoreIdSerializer;
	//    
//	    public void setJsonStoreIdSerializer(JsonSerializer<StoreId> jsonStoreIdSerializer) {
...
//	    }
	    ...
	    public void setJsonSerializers(List<JsonSerializer<?>> jsonSerializers) {
	        Assert.notNull(jsonSerializers, "you must set the json serilizers for Objectmppaer facotry.");
...
//	            logger.debug("==========return a object mapper================ with address: {}" , objectMapper);
	            return objectMapper;
	        }
	        ...
	        serializationConfig = serializationConfig.withDateFormat(StdDateFormat.getBlueprintISO8601Format());
	        serializationConfig = serializationConfig.without(Feature.WRITE_DATES_AS_TIMESTAMPS);
	        serializationConfig = serializationConfig.without(Feature.WRITE_NULL_MAP_VALUES);//禁止Map的空值序列化
	        serializationConfig = serializationConfig.without(Feature.FAIL_ON_EMPTY_BEANS);
	        ...
	        deserializationConfig = deserializationConfig.without(DeserializationConfig.Feature.FAIL_ON_NULL_FOR_PRIMITIVES);//
	        ...
	        //添加对地理Point的序列化类
	        SimpleModule module = new SimpleModule("Map Point Serializer", new Version(1, 0, 0, "Final"));
